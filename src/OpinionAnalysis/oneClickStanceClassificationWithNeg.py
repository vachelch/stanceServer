...
def polarityNeg(textStr, polarityList):
	polarityIden = 1
	textStrLower = textStr
	for word in polarityList:
		if word in textStrLower:
			polarityIden *= -1
		textStrLower.replace(word, "").strip()
	return textStrLower, polarityIden

def cutSent(textStr):
	sents = re.split('[.!?。？！ ]', textStr)
	return [ele for ele in sents if len(ele) > 0]
...
def matchFunc(titleText, bodyText, vocabList, word2vecModel):
	...
	titleText, claimPolarity = polarityNeg(titleText, polarityList)
	...
	# count article embedding
	# Output the cos sim of the embedding of the word in body text and 
	# the embedding of title(word embedding sum)
	bodySents = cutSent(bodyText)
	newBodyText = ""
	wordCounter1 = 0
	bodyEmbedding = list(zeroEmbedding)
	for index in range(len(bodySents)):
		tmpText, tmpPolarity = polarityNeg(bodySents[index], polarityList)
		newBodyText = "{} {}".format(newBodyText, tmpText)
		tmpEmbedding = _matchFunc(titleEmbedding, tmpText, vocabList, word2vecModel)
		if tmpPolarity*claimPolarity < 0:
			tmpEmbedding = [-1*ele for ele in tmpEmbedding]

		if not checkAllZero(tmpEmbedding):
			wordCounter1 += 1
			bodyEmbedding = np.add(bodyEmbedding, tmpEmbedding)
	if wordCounter1 > 0:
		bodyEmbedding = np.array(bodyEmbedding)/wordCounter1
	resultList.append(bodyEmbedding)

	bodyWordList = cutWord(newBodyText).split()
	resultList.append([min(word2vecModel.wmdistance(titleWordList, bodyWordList), _MAX_wmd)])
	if checkAllZero(titleEmbedding) or checkAllZero(bodyEmbedding):
		resultList.append([0.0])
	else:
		resultList.append([1-spatial.distance.cosine(titleEmbedding, bodyEmbedding)])
	return np.concatenate(resultList, axis=0)

def _matchFunc(titleEmbedding, bodyText, vocabList, word2vecModel):
	weightList = list()
	zeroEmbedding = [0.0]*len(word2vecModel["這"])
...
def stanceAnalyzerCos(queryText, bodyText, w2v):
	...
	# modelFilename = "/dhome/b01901130/stanceEmbedding/Chinese-Discovery-News-master/src/OpinionAnalysis/XGBoost3_F0_Att2ChineseWeighted15Fold_v8WithNEG_WOTitleEP2000.pickle"
	modelFilename = "/dhome/b01901130/stanceEmbedding/Chinese-Discovery-News-master/src/OpinionAnalysis/XGBoost3_F0_Att2ChineseWeighted15Fold_v8WithNEGEP2000.pickle"
...
def stanceAnalyzer(queryText, bodyText, w2v):
	...
	# modelCosFilename = "/dhome/b01901130/stanceEmbedding/Chinese-Discovery-News-master/src/OpinionAnalysis/XGBoost3_F0_Att2ChineseWeighted15Fold_v8WithNEG_WOTitleEP2000.pickle"
